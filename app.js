...
function renderDashboard(){
 ...
 if(latest7){
   const lp=[...(latest7.results||[])].sort((a,b)=>b.rank-a.rank)[0];
   $("#lastPointCard").innerHTML=`<div class="last-point-visual"><div class="last-point-avatar">🎴</div><div><span class="eyebrow">LAST POINT PLAYER</span><h3>${esc(lp?.name||"—")}</h3><p>7s Point • Round 7 completed • Ready for next game</p></div><strong class="last-point-score">${lp?.score??"—"}</strong></div>`;
 }else{
...
function renderSevenPanel(){
 ...
 <div class="round-rule">${doubled?"Whatever you enter is doubled (e.g. 5 → 10)":"Points are entered as-is (e.g. 5 → 5)"} <span>• Enter this round's points</span></div>
...
function saveSevenRound(){
 ...
 if(round===7){
   ...
   const lastPointPlayerId=results[results.length-1]?.memberId||null;
   state.games.push({id:id(),type:"7s Point",date:new Date().toISOString(),results,rounds:7,roundBreakdown:d.rounds,lastPointPlayerId});
